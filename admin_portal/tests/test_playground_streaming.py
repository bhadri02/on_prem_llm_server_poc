"""
admin_portal/tests/test_playground_streaming.py

Tests for POST /portal/playground/chat's streaming path (stream=true) —
mirrors test_chat_streaming.py, since both routes now share
services/proxy.py's sse_relay_with_inband_error. The non-streaming path is
unaffected (existing behavior, not modified beyond adding the `stream`
field default False, already covered by test_playground_chat.py-style
tests elsewhere).
"""
...
class TestPlaygroundStreamingRelay:
    async def test_relays_upstream_sse_bytes_unchanged(self, client):
        ...
        with respx.mock(assert_all_called=True) as mock:
            ...
            response = await client.post("/portal/playground/chat", json=_REQUEST_BODY)
