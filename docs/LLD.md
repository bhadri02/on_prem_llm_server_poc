...
### 9.1 `api_gateway` audit events never reach the durable trail
`auth_fail`, `auth_pass`, `rate_limited`, `request_received`, `response_sent` are stdout-only (`api_gateway/services/audit.py`). Only `security_layer` and `intelligent_router` POST to `audit_store`. Consequence: 401/403/429 gateway-layer rejections are invisible to `GET /portal/governance/summary` and `GET /portal/audit/events`. Fix, if prioritized: make `api_gateway` POST via the same fire-and-forget `BackgroundTasks` pattern the other two layers already use.
...
Predates RBAC/Postgres/policy-matrix/governance work entirely — `router`'s chart has `env: {}` (none of `MODEL_MATRIX_PATH`, `ADMIN_PORTAL_INTERNAL_KEY`, `POLICY_MATRIX_PATH` wired), no Postgres dependency anywhere in the chart tree, and no `portal_ui` chart despite a working Dockerfile. Deploying via `scripts/deploy.sh` today would crash-loop most pods. **The Docker Compose path (`docker-compose.prod.yml`, `docs/DEPLOYMENT.md`) is the current, validated deployment path** — built, deployed, and end-to-end tested (chat completion, injection block, cache hit, governance data) against the current codebase, including catching and fixing four real bugs in the process (see `docs/DEPLOYMENT.md`'s "Bugs found and fixed" section).

### 9.4 `intelligent_router::classify_task()` has the same "joins full history" bug the cache key had
Not yet fixed — flagged, not actioned.
